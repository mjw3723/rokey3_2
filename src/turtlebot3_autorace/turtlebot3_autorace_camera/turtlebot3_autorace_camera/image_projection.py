...
class ImageProjection(Node):
    ...
    def cbImageProjection(self, msg_img):
        ...
        cv_image_original = cv2.resize(cv_image_original, (self.target_width, self.target_height), 
        ...
        if self.is_calibration_mode:
            ...
            cv_image_calib = cv2.line(
                cv_image_calib,
                (320 - top_x, 180 - top_y), # 변경됨: 160->320, 180->360
                (320 + top_x, 180 - top_y), # 변경됨: 160->320, 180->360
                (0, 0, 255),
                1
            )
            cv_image_calib = cv2.line(
                cv_image_calib,
                (320 - bottom_x, 120 + bottom_y), # 변경됨: 160->320, 120->240
                (320 + bottom_x, 120 + bottom_y), # 변경됨: 160->320, 120->240
                (0, 0, 255),
                1
            )
        ...
        pts_src = np.array([
            [320 - top_x, 180 - top_y],
            [320 + top_x, 180 - top_y],
            [320 + bottom_x, 120 + bottom_y],
            [320 - bottom_x, 120 + bottom_y]
        ])

        # selecting 4 points from image that will be transformed
        pts_dst = np.array([[75, 0], [875, 0], [875, 600], [75, 600]])
        # findiaz homography matrix
