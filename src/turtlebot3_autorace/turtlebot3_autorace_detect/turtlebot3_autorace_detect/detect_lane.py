...
class DetectLane(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameters(
            ...
            parameters=[
                ...
                ('detect.lane.white.hue_h', 255, #179
                    parameter_descriptor_hue),
                ('detect.lane.white.saturation_l', 0,
                    parameter_descriptor_saturation_lightness),
                ('detect.lane.white.saturation_h', 255, # 79
                    parameter_descriptor_saturation_lightness),
                ('detect.lane.white.lightness_l', 0, #105
                    parameter_descriptor_saturation_lightness),
        ...
        #         )
            ...
            self.sub_image_original = self.create_subscription(
                Image, '/image_raw', self.cbFindLane, 1
                )
        ...
        self.test_image = self.create_publisher(
            Image,'/test',1
        )
        ...
        self.left_fit = None
        self.right_fit = None # 이전에 발생했던 right_fit 문제도 같이 해결됩니다.

        # 이전 프레임의 차선 정보를 저장할 변수 (sliding_windown 함수에서 사용될 수 있음)
        self.lane_fit_bef = None # 이전에 발생했던 lane_fit_bef 문제도 같이 해결됩니다.
        self.left_old_fit = np.array([0., 0., 0.]) # 차선 계수 3개짜리 배열로 초기화
        self.right_old_fit = np.array([0., 0., 0.]) # 차선 계수 3개짜리 배열로 초기화
    ...
    def cbFindLane(self, image_msg):
        ...
        self.brightness = self.get_brightness(cv_image)
        #self.get_logger().info(f'[Auto Gamma] brightness: {self.brightness}')

        # if self.brightness > 170:
        #     gamma = 2.0
        # elif self.brightness > 140:
        #     gamma = 1.8
        # elif self.brightness > 110:
        #     gamma = 1.5
        # else:
        #     gamma = 1.2
        ...
        #cv_image = self.gaussian_blue(cv_image,5)
        # ros_image_msg = self.cvBridge.cv2_to_imgmsg(cv_image, "bgr8")
        # self.test_image.publish(ros_image_msg)
        # self.brightness = self.get_brightness(cv_image)
        # self.get_logger().info(f'brightness state: {self.brightness}')
        ...
        #     self.get_logger().warn("Initial lane fitting failed. Waiting for next frame.")
        #     return
        try:
            if yellow_fraction > 3000:
                ...
                #     self.mov_avg_left, np.array([self.left_fit]), axis=0
                    # )
                self.left_fitx, new_fit = self.fit_from_lines(self.left_fit, cv_yellow_lane)
                if self.is_curve_reasonable(new_fit,self.left_old_fit):
                    self.left_old_fit = new_fit
                    self.left_fit = new_fit
                    self.mov_avg_left = np.append(self.mov_avg_left, np.array([new_fit]), axis=0)
                else:
                    self.mov_avg_left = np.append(self.mov_avg_left, np.array([self.left_old_fit]), axis=0)
            ...
            if white_fraction > 3000:
                ...
                #     )
                self.right_fitx, new_fit = self.fit_from_lines(self.right_fit, cv_white_lane)
                if self.is_curve_reasonable(new_fit,self.right_old_fit):
                    self.right_old_fit = new_fit
                    self.right_fit = new_fit
                    self.mov_avg_right = np.append(self.mov_avg_right, np.array([new_fit]), axis=0)
                else:
                    self.mov_avg_right = np.append(self.mov_avg_right, np.array([self.right_old_fit]), axis=0)
        except Exception:
            if yellow_fraction > 3000:
        ...
        if self.mov_avg_left.shape[0] > 640:
            self.mov_avg_left = self.mov_avg_left[0:MOV_AVG_LENGTH]

        if self.mov_avg_right.shape[0] > 640:
            self.mov_avg_right = self.mov_avg_right[0:MOV_AVG_LENGTH]

        self.make_lane(cv_image, white_fraction, yellow_fraction)

    def is_curve_reasonable(self, fit, old_fit):
        curvature = fit[0]
        prev_curvature = old_fit[0]
        # 2차항 계수 값이 너무 크거나 작은 경우 제거
        if abs(curvature) > 0.001: #0.001
            self.get_logger().warn(f'[WARNING] Curvature {curvature:.6f} is out of acceptable range')
            return False
        # 급격한 변화 필터링
        if abs(curvature - prev_curvature) > 7e-4: #5e-5
            self.get_logger().warn(f'[WARNING] Curvature change too large: {curvature:.6f} -> {prev_curvature:.6f}')
            return False
        return True
    ...
    def maskWhiteLane(self, image):
        ...
        hsv = self.equalize_sv_channel_for_masking(image)
        #mask = cv2.inRange(hsv_eq, lower_bound, upper_bound)
        if  self.brightness >= 145:
            lower_white = np.array([0, 0, 230])
            upper_white = np.array([180, 50, 255])
        elif 140 <= self.brightness < 145:
            lower_white = np.array([0, 0, 230])
            upper_white = np.array([180, 50, 255])
        elif 130 <= self.brightness < 140:
            lower_white = np.array([0, 0, 220])
            upper_white = np.array([180, 50, 255])
        elif 120 <= self.brightness < 130:
            lower_white = np.array([0, 0, 200])
            upper_white = np.array([180, 10, 255])
        elif 110 <= self.brightness < 120:
            lower_white = np.array([0, 0, 200])
            upper_white = np.array([180, 50, 255])
        elif 105 <= self.brightness < 110:
            lower_white = np.array([0, 0, 200])
            upper_white = np.array([180, 50, 255])
        elif 60 <= self.brightness < 105:
    ...
    def maskYellowLane(self, image):
        ...
        if self.brightness >= 145:
            lower_yellow = np.array([15, 0, 210])
            upper_yellow  = np.array([35, 255, 255])
        elif 140 <= self.brightness< 145:
            lower_yellow = np.array([20, 30, 210])
            upper_yellow  = np.array([75, 250, 255])
        elif 130 <= self.brightness < 140:
            lower_yellow = np.array([20, 25, 210])
    ...
    def make_lane(self, cv_image, white_fraction, yellow_fraction):
        ...
        if self.reliability_white_line > 50 and self.reliability_yellow_line > 50:
            ...
            if white_fraction > 3000 and yellow_fraction <= 3000:
                centerx = np.subtract(self.right_fitx, 179)
                pts_center = np.array([np.transpose(np.vstack([centerx, ploty]))])
            ...
            if white_fraction <= 3000 and yellow_fraction > 3000:
                centerx = np.add(self.left_fitx, 179)
                pts_center = np.array([np.transpose(np.vstack([centerx, ploty]))])
            ...
            self.is_center_x_exist = False
            self.get_logger().info(f'state ================== 0')
            lane_state.data = 0
